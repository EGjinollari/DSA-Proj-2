...
void IDRBTree::Insert(int id, std::string fn, std::string ln, int bday, std::string origin, std::string dest){
    {
...
Node* IDRBTree::Insert_Helper(Node* root, Person* p, std::string val, Node*& newNode){
    if (!root){
        Node* n = new Node(p);
        newNode = n;
        return n;
...
Node* IDRBTree::Delete_Helper(Node* root, Person* target, Node*& successor, bool& color) {
    ...
    int curr = stoi(root->get_data()->get_id()); 
    if (stoi(target->get_id()) < curr){
